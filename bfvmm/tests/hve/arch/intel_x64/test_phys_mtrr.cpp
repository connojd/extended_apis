...
TEST_CASE("phys_mtrr: mem_type - variable throws")
{
    enable_mtrr();
    ...
    physmask::valid::enable(physmask);

    msrs_n::set(physmask::start_addr, physmask);
    std::array<uint64_t, 3U> invalid_mtrr_type = {2U, 3U, 7U};

    for (auto type : invalid_mtrr_type) {
        physbase::type::set(physbase, type);
        msrs_n::set(physbase::start_addr, physbase);
        CHECK_THROWS(phys_mtrr());
    }
