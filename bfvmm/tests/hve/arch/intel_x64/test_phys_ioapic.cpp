...
TEST_CASE("phys_ioapic")
{
    ...
    CHECK_THROWS(ioapic.read_register(offset));
    CHECK_THROWS(ioapic.write_register(offset, 0U));

    for (eapis::intel_x64::ioapic::offset_t i = 0U; i < 0x40U; ++i) {
        if (eapis::intel_x64::ioapic::is_writable(i)) {
            CHECK_NOTHROW(ioapic.write_register(i, 0xFFFFFFFFU));
            CHECK(eapis::intel_x64::ioapic::is_readable(i));
            CHECK_NOTHROW(ioapic.read_register(i) == 0xFFFFFFFFU);
            continue;
        }

        if (eapis::intel_x64::ioapic::is_read_only(i)) {
            CHECK_THROWS(ioapic.write_register(i, 0U));
            CHECK_NOTHROW(ioapic.read_register(i));
            continue;
        }
