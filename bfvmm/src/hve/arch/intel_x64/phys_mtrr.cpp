...
phys_mtrr::phys_mtrr()
{
    ...
    if (need_print) {
        bfdebug_text(
            0,
            "MTRR default type: ",
            type_to_str(ia32_mtrr_def_type::type::get(def)));

        if (this->fixed_enabled()) {
            bfdebug_info(0, "MTRR fixed ranges enabled:");
            this->print_fixed_ranges(0);
        }
        else {
            bfdebug_info(0, "MTRR fixed ranges disabled");
        }

        bfdebug_info(0, "MTRR variable ranges enabled:");
        this->print_variable_ranges(0);
        need_print = false;
    }
}

static uint64_t fixed_range_size(uint64_t range)
{
    expects(range <= 87U);
...
phys_mtrr::print_variable_ranges(uint64_t level) const
{
    for (auto i = 0U; i < m_variable_range.size(); ++i) {
        bfdebug_text(level, "type", type_to_str(m_variable_range[i].type()));
...
phys_mtrr::print_fixed_ranges(uint64_t level) const
{
    ...
    for (auto i = 0U; i < m_fixed_type.size(); ++i) {
        ...
        bfdebug_text(level, "type", type_to_str(m_fixed_type[i - 1U]));
    ...
    if (type == m_fixed_type[m_fixed_type.size() - 1U]) {
        bfdebug_text(level, "type", type_to_str(type));
        bfdebug_subnhex(level, "base", base);
        bfdebug_subnhex(level, "last", base + (size - 1U));
    }
}
...
/// {0..87} -> {memory types} is provided in another array.
///
/// As a plus, once -std=c++17 is fully integrated, this function may be marked
/// constexpr.
///
void
