...
#define PHYS_MEM_SIZE (1ULL << 37U) // Arbitrary max mem
#endif
...
phys_mtrr::phys_mtrr()
{
    ...
    bfdebug_transaction(0, [&](std::string *){
        this->print_fixed_ranges(0);
        this->print_variable_ranges(0);
        this->print_range_list(0);
    });
}
...
phys_mtrr::parse_fixed_mtrrs()
{
    for (uint8_t i = 0U; i <= 0xFFU; ++i) {
        if (i <= 0x7FU) {
            ...
            const uint8_t mod = 4U;
            const uint8_t base = 8U;
            const uint8_t index = ((i & 0x0FU) >> 2U);
            const uint8_t scale = ((i & 0xF0U) >> 4U) & (mod - 1U);
            m_fixed_range.at(i) = base + index + (scale * mod);
        }
        else {
            const uint8_t mod = 16U;
            const uint8_t base = 24U;
            const uint8_t index = i & 0x0FU;
            const uint8_t scale = (((i & 0xF0U) >> 4U) & (mod - 1U)) - 0xCU;
            m_fixed_range.at(i) = base + index + (scale * mod);
        }
    }

    for (uint8_t i = 0U; i < fixed_addr.size(); ++i) {
        const uint64_t msr = ::intel_x64::msrs::get(fixed_addr.at(i));
        for (uint8_t j = 0U; j < 8U; ++j) {
            const uint8_t from = (j << 3U);
            const uint64_t mask = 0xFFULL << from;
            const uint8_t type = gsl::narrow_cast<uint8_t>(get_bits(msr, mask) >> from);
...
phys_mtrr::parse_variable_mtrrs()
{
    ...
    const uint64_t base_addr = ia32_physbase::start_addr;
    const uint64_t mask_addr = ia32_physmask::start_addr;

    for (uint64_t i = 0U; i < (vcnt << 1U); i += 2U) {
...
phys_mtrr::setup_variable_range_list()
{
    ...
    if (base < PHYS_MEM_SIZE) {
        size = PHYS_MEM_SIZE - base;
        type = ia32_mtrr_def_type::type::get(m_def);
        m_range_list.push_back({base, size, type});
    }
}
