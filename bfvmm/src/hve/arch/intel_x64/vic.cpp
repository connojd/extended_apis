...
static auto invept_eptp(::intel_x64::vmx::eptp_type eptp)
{ ::intel_x64::vmx::invept_single_context(eptp); }
    ...
    uintptr_t xapic_hpa)
{
    ...
    const auto hi_end = 0x500000000ULL - ept::page_size_1g;

    ept::identity_map_bestfit_lo(emm, 0ULL, lo_end);
    ept::map_4k(emm, xapic_gpa, xapic_hpa, ept::epte::memory_attr::wb_pt);
...
vic::vic(
    ...
    m_need_write_mtf{false},
    m_need_read_mtf{false},
    m_virt_base_msr{0ULL},
    m_phys_base_msr{0ULL},
    m_orig_base_msr{::intel_x64::msrs::ia32_apic_base::get()},
    m_access_offset{0ULL},
    m_hve{hve},
    m_emm{emm}
{
    if (hve->vmcs()->save_state()->vcpuid == 2) {
        this->init_idt();
        this->init_lapic();
...
vic::init_phys_xapic()
{
    ...
    if (map == nullptr) {
        throw_vic_fatal("init_phys_xapic: unable to map xAPIC page into VMM");
    }
...
vic::init_virt_lapic()
{
    ...
    m_apic_access = std::make_unique<gsl::byte[]>(4096U);
    m_virt_lapic = std::make_unique<virt_lapic>(
        m_hve, bfrcast(uint32_t *, m_apic_access.get()), m_phys_lapic.get()
    );
}
...
vic::add_xapic_handlers()
{
    ...
    auto shadow_hpa{0ULL};

    if (vmcs_n::apic_access_address::exists()) {
        shadow_hpa = g_mm->virtptr_to_physint(m_apic_access.get());
        vmcs_n::apic_access_address::set(shadow_hpa);
        proc_ctls2::virtualize_apic_accesses::enable();
        init_xapic_ept(m_hve, *m_emm, xapic_gpa, shadow_hpa);

        m_hve->exit_handler()->add_handler(
            vmcs_n::exit_reason::basic_exit_reason::apic_access,
            ::handler_delegate_t::create<vic, &vic::handle_apic_access>(this));

        m_hve->add_monitor_trap_handler(
            monitor_trap::handler_delegate_t::create<vic,
            &vic::handle_xapic_write_mtf>(this));

        m_hve->add_monitor_trap_handler(
            monitor_trap::handler_delegate_t::create<vic,
            &vic::handle_xapic_read_mtf>(this));
    }
...
bool
vic::handle_apic_access(gsl::not_null<vmcs_t *> vmcs)
{
    bfignored(vmcs);
    using namespace vmcs_n::exit_qualification;

    const auto qual = get();
    const auto byte_offset = linear_apic_access::offset::get(qual);
    const auto apic_access = linear_apic_access::access_type::get(qual);

    // The qual holds the *byte* offset into the apic page. We treat the
    // byte_offset as an address, and convert it into a canonical offset
    // This is OK because the pfn gets masked to 0 in the conversion.
    m_access_offset = lapic_register::mem_addr_to_offset(byte_offset);

    switch (apic_access) {
    case linear_apic_access::access_type::read_during_instruction_execution:
        m_need_read_mtf = true;
        m_need_write_mtf = false;
        break;

    case linear_apic_access::access_type::write_during_instruction_execution: {
        const auto addr = lapic_register::offset_to_msr_addr(m_access_offset);
        if (addr == ::intel_x64::msrs::ia32_x2apic_eoi::addr) {
            m_virt_lapic->write_eoi();
            return advance(vmcs);
        }
        m_need_write_mtf = true;
        m_need_read_mtf = false;
    }   break;

    default:
        throw_vic_fatal("unsupported apic-access type:", apic_access);
    }

    m_hve->enable_monitor_trap_flag();
    proc_ctls2::virtualize_apic_accesses::disable();

    return true;
}

bool
vic::handle_xapic_write_mtf(
    gsl::not_null<vmcs_t *> vmcs, monitor_trap::info_t &info)
{
    ...
    const auto val = m_virt_lapic->read_register(m_access_offset);
    m_phys_lapic->write_register(m_access_offset, val);
    m_need_write_mtf = false;
    proc_ctls2::virtualize_apic_accesses::enable();
...
//{
//    using namespace ::intel_x64::msrs;
//    namespace abm = ::intel_x64::msrs::ia32_apic_base::apic_base;
//    bfignored(vmcs);
//
//    const auto offset = lapic_register::mem_addr_to_offset(info.gpa);
//    if (lapic_register::offset_to_msr_addr(offset) == ia32_x2apic_eoi::addr) {
//        return true;
//    }
//
//    info.ignore_advance = true;
//    m_xapic_write_mtf = true;
//    m_xapic_write_offset = offset;
//    m_hve->enable_monitor_trap_flag();
//    auto &entry = m_emm->gpa_to_epte(abm::get(m_virt_base_msr));
//    ept::epte::write_access::enable(entry);
//    invept_eptp(ept::eptp(*m_emm));
//
//    return true;
//}
