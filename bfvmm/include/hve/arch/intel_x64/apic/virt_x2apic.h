...
public:
    ...
    ///
    uint64_t read_register(uint32_t addr) const;
    ...
    ///
    void write_register(uint32_t addr, uint64_t val);
...
private:
    ...
    void reset_svr();
    void reset_version();
    void reset_register(uint32_t offset);
    void reset_lvt_register(uint32_t offset);
    void clear_register(uint32_t offset);
    ...
    std::unordered_map<uint32_t, uint64_t> m_reg;
