...
extern std::array<
    lapic_register::value_type, lapic_register::count> lapic_registers;
...
namespace lapic_register
{
    ...
    constexpr inline auto offset_to_mem_addr(
        uint64_t offset, uintptr_t base = ::intel_x64::lapic::xapic_default_base)
    { return base | (offset << 4U); }

    constexpr inline auto offset_to_msr_addr(uint64_t offset)
    ...
    inline auto exists_in_x2apic(uint64_t offset)
    {
        const auto reg = lapic_registers.at(offset);

        return lapic_register::x2apic_readable::is_enabled(reg) ||
               lapic_register::x2apic_writable::is_enabled(reg);
    }

    inline auto readable_in_x2apic(uint64_t offset)
    {
        const auto reg = lapic_registers.at(offset);
        return lapic_register::x2apic_readable::is_enabled(reg);
    }

    inline auto writable_in_x2apic(uint64_t offset)
    {
        const auto reg = lapic_registers.at(offset);
