...
init_nonexistent(uint64_t offset) noexcept
{
    ...
    reg = lapic_register::xapic_readable::disable(reg);
    reg = lapic_register::xapic_writable::disable(reg);

    reg = lapic_register::x2apic_readable::disable(reg);
    reg = lapic_register::x2apic_writable::disable(reg);

    lapic_registers[offset] = reg;
}

void
init_xapic_read_write(uint64_t offset) noexcept
{
    auto reg = lapic_registers[offset];

    reg = lapic_register::x2apic_readable::disable(reg);
    reg = lapic_register::x2apic_writable::disable(reg);

    reg = lapic_register::xapic_readable::enable(reg);
    reg = lapic_register::xapic_writable::enable(reg);
...
init_x2apic_write_only(uint64_t offset) noexcept
{
    ...
    reg = lapic_register::x2apic_readable::disable(reg);
    reg = lapic_register::x2apic_writable::enable(reg);
...
init_both_read_only(uint64_t offset) noexcept
{
    ...
    reg = lapic_register::x2apic_readable::enable(reg);
    reg = lapic_register::x2apic_writable::disable(reg);
...
init_lapic_registers() noexcept
{
    ...
    for (auto i = 0U; i < lapic_registers.size(); i++) {
        switch (i) {
