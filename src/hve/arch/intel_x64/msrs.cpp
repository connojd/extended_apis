...
msrs::~msrs()
{
    if(m_rdmsr_log_enabled) {
        dump_rdmsr_log();
    }

    if(m_wrmsr_log_enabled) {
        dump_wrmsr_log();
    }
}
...
msrs::handle_rdmsr(gsl::not_null<bfvmm::intel_x64::vmcs *> vmcs)
{
    ...
    if (GSL_UNLIKELY(m_rdmsr_log_enabled)) {
        m_rdmsr_log[msr]++;
    }
...
msrs::handle_wrmsr(gsl::not_null<bfvmm::intel_x64::vmcs *> vmcs)
{
    ...
    if (GSL_UNLIKELY(m_wrmsr_log_enabled)) {
        m_wrmsr_log[msr]++;
    }
...
{ m_rdmsr_log_enabled = true; }

void
msrs::disable_rdmsr_log()
{ m_rdmsr_log_enabled = false; }
...
msrs::dump_rdmsr_log()
{
    bfdebug_transaction(0, [&](std::string * msg) {
        ...
        for(const auto &elem : m_rdmsr_log) {
            bfdebug_subnhex(0, bfn::to_string(elem.first, 16).c_str(), elem.second, msg);
        }
...
void
msrs::enable_wrmsr_log()
{ m_wrmsr_log_enabled = true; }

void
msrs::disable_wrmsr_log()
{ m_wrmsr_log_enabled = false; }
...
msrs::dump_wrmsr_log()
{
    bfdebug_transaction(0, [&](std::string * msg) {
        ...
        for(const auto &elem : m_wrmsr_log) {
            bfdebug_subnhex(0, bfn::to_string(elem.first, 16).c_str(), elem.second, msg);
        }
